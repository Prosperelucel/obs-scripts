...
while True:
    ...
    if command:
        packet01 = requests.SetSceneItemPosition(scene_name=scene, item=source_name, x=old_pos_x, y=old_pos_y)
        t1 = Thread(target=sendcall, args=(packet01,))
        t1.start()
        packet02 = requests.SetSceneItemTransform(scene_name=scene, item=source_name, x_scale=old_scale_x,y_scale=old_scale_y, rotation=old_rotation)
        t2 = Thread(target=sendcall, args=(packet02,))
        t2.start()
    ...
    if joystick_count > 0:
        ...
        if axis_rotation < -0.1:
            rotation_up = False
            rotation_down = True
        if axis_rotation > -0.1 and axis_rotation < 0.1:
            rotation_down = False
            rotation_up = False
        ...
        if axis_x > 0.1 or dpad_x > 0.1: #Right
            droite = True
            gauche = False
        if axis_x < -0.1 or dpad_x < -0.1: #Left
            droite = False
            gauche = True
        if axis_y < -0.1 or dpad_y > 0.1: #Top
            haut = True
            bas = False
        if axis_y > 0.1 or dpad_y < -0.1: #Bot
            haut = False
            bas = True
        if axis_y < 0.1 and axis_y > -0.1 and axis_x < 0.1 and axis_x > -0.1 and dpad_x < 0.1 and dpad_x > -0.1 and dpad_y < 0.1 and dpad_y > -0.1: #center
            haut = False
            bas = False
            droite = False
            gauche = False
